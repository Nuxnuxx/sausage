...
namespace NHP_Domain.UseCases.TypeDestinationUseCases.Update;

public class UpdateTypeDestinationUseCase(IRepositoryFactory repositoryFactory)
{
    public async Task<TypeDestination?> ExecuteAsync(TypeDestination typeDestination)
    {
        await CheckBusinessRules(typeDestination);
        await repositoryFactory.TypeDestinationRepository().UpdateAsync(typeDestination);
